export type GuideCollisionReferenceEntry = {
  entryName: string;
...
export type LinkDynamicsPatch = {
  mass?: number;
  centerOfMassLocal?: [number, number, number];
  diagonalInertia?: [number, number, number];
  principalAxesLocalWxyz?: [number, number, number, number];
};

export type XformOpValue =
  | number
...
export function normalizeUsdPathToken(path: string | null | undefined): string {
...
export function extractUsdAssetReferencesFromLayerText(
  ...
  while ((match = assetRegex.exec(layerText))) {
    ...
    if (baseOnly && !/base/i.test(assetPath)) continue;
    paths.add(assetPath);
  }
  return Array.from(paths);
}

export function extractReferencePrimTargets(value: string | null | undefined): string[] {
  const source = String(value || "");
  if (!source) return [];

  const targets: string[] = [];
  const addTarget = (candidate: string | null | undefined) => {
...
export function extractScopeBodyText(layerText: string | null | undefined, scopeName: string | null | undefined): string {
  if (!layerText || !scopeName) return "";
...
export function parseVisualSemanticChildNamesFromLayerText(layerText: string | null | undefined): Map<string, string[]> {
  const visualsScopeText = extractScopeBodyText(layerText, "visuals");
  if (!visualsScopeText) return new Map();

  const linkToChildNames = new Map<string, string[]>();
  const stack: Array<{ name: string }> = [];
  let pendingContextName: string | null = null;

  const addChildName = (linkName: string, childName: string) => {
    const normalizedLinkName = String(linkName || "").trim();
    const normalizedChildName = String(childName || "").trim();
    if (!normalizedLinkName || !normalizedChildName) return;

    const existingNames = linkToChildNames.get(normalizedLinkName) || [];
    if (existingNames.includes(normalizedChildName)) return;
    ...
    for (const character of line) {
      if (character === "{") {
        ...
        const hierarchy = stack
          ...
          .filter((name) => !!name);
        if (hierarchy.length < 3) continue;
        if (String(hierarchy[0] || "").toLowerCase() !== "visuals") continue;
...
export function parseGuideCollisionReferencesFromLayerText(layerText: string | null | undefined): Map<string, GuideCollisionReferenceEntry[]> {
  const collidersText = extractScopeBodyText(layerText, "colliders");
  if (!collidersText) return new Map();

  const linkToGuideEntries = new Map<string, GuideCollisionReferenceEntry[]>();
  const stack: Array<{ name: string; hasGuidePurpose: boolean; referencePaths: string[] }> = [];
  let pendingContext: { name: string; hasGuidePurpose: boolean; referencePaths: string[] } | null = null;

  const addGuideEntry = (linkName: string, entryName: string, referencePath: string | null) => {
    if (!linkName || !entryName) return;
    const existing = linkToGuideEntries.get(linkName) || [];
    if (existing.some((entry) => entry.entryName === entryName && entry.referencePath === referencePath)) return;
    existing.push({ entryName, referencePath });
    linkToGuideEntries.set(linkName, existing);
  };

  const applyMetadataLine = (context: { hasGuidePurpose: boolean; referencePaths: string[] } | null, line: string) => {
  ...
  const getCurrentLinkName = (parentNames: string[], poppedName: string | null) => {
  ...
  for (const line of lines) {
    ...
    for (const character of line) {
      if (character === "{") {
        ...
        if (!poppedContext || !poppedContext.name || !poppedContext.hasGuidePurpose) continue;
        ...
        if (!linkName || linkName === "colliders") continue;
...
export function parseColliderEntriesFromLayerText(layerText: string | null | undefined): Map<string, ColliderEntry[]> {
  const collidersText = extractScopeBodyText(layerText, "colliders");
  if (!collidersText) return new Map();

  const linkToColliderEntries = new Map<string, ColliderEntry[]>();
  const stack: Array<{ name: string }> = [];
  let pendingContextName: string | null = null;
  const allowedGeometryNames = new Set(["mesh", "box", "cube", "sphere", "cylinder", "capsule"]);

  const addColliderEntry = (linkName: string, entryName: string) => {
    const normalizedLinkName = String(linkName || "").trim();
    const normalizedEntryName = String(entryName || "").trim();
    if (!normalizedLinkName || !normalizedEntryName) return;
    if (normalizedLinkName === "colliders") return;
    ...
    for (const character of line) {
      if (character === "{") {
        ...
        const hierarchy = stack
          .map((context) => String(context?.name || "").trim())
          .filter((name) => !!name);
        if (hierarchy.length < 4) continue;
        if (String(hierarchy[0] || "").toLowerCase() !== "colliders") continue;

        const geometryName = String(hierarchy[hierarchy.length - 1] || "").toLowerCase();
        if (!allowedGeometryNames.has(geometryName)) continue;
...
function parseVector3FromTupleLiteral(tupleLiteral: string | null | undefined): [number, number, number] | null {
...
function normalizeQuaternionWxyzTuple(tupleValue: number[]): [number, number, number, number] | null {
  if (!Array.isArray(tupleValue) || tupleValue.length < 4) return null;
  const w = Number(tupleValue[0]);
  const x = Number(tupleValue[1]);
  const y = Number(tupleValue[2]);
  const z = Number(tupleValue[3]);
  if (![w, x, y, z].every((value) => Number.isFinite(value))) return null;
  const length = Math.hypot(w, x, y, z);
  if (!Number.isFinite(length) || length <= 1e-12) return null;
  return [w / length, x / length, y / length, z / length];
}

function parseQuaternionWxyzFromTupleLiteral(tupleLiteral: string | null | undefined): [number, number, number, number] | null {
  if (!tupleLiteral) return null;
  const source = String(tupleLiteral || "")
    .split(",")
    .map((part) => Number(part.trim()))
...
function normalizeAxisToken(value: string | null | undefined): "X" | "Y" | "Z" {
...
export function extractJointRecordsFromLayerText(layerText: string | null | undefined): JointRecord[] {
  if (!layerText || typeof layerText !== "string") return [];
...
function countBracesOutsideStrings(source: string): { openCount: number; closeCount: number } {
  ...
  for (let cursor = 0; cursor < source.length; cursor += 1) {
    ...
    else if (character === "}") closeCount += 1;
  }
  return { openCount, closeCount };
}

function composeChildPrimPath(parentPrimPath: string | null, childPrimName: string | null | undefined): string {
...
function ensureLinkDynamicsPatch(target: Map<string, LinkDynamicsPatch>, linkPath: string): LinkDynamicsPatch | null {
...
export function parseLinkDynamicsPatchesFromLayerText(layerText: string | null | undefined): Map<string, LinkDynamicsPatch> {
  const patchesByLinkPath = new Map<string, LinkDynamicsPatch>();
  if (!layerText || typeof layerText !== "string") return patchesByLinkPath;
    ...
    if (currentPrimPath) {
      ...
      if (centerOfMassMatch) {
        ...
        if (centerOfMassLocal) {
          const patch = ensureLinkDynamicsPatch(patchesByLinkPath, currentPrimPath);
          if (patch) patch.centerOfMassLocal = centerOfMassLocal;
        }
      }
      ...
      if (diagonalInertiaMatch) {
        ...
        if (diagonalInertia) {
          const patch = ensureLinkDynamicsPatch(patchesByLinkPath, currentPrimPath);
          if (patch) patch.diagonalInertia = diagonalInertia;
        }
      }
      ...
      if (principalAxesMatch) {
        ...
        if (principalAxesLocalWxyz) {
          const patch = ensureLinkDynamicsPatch(patchesByLinkPath, currentPrimPath);
          if (patch) patch.principalAxesLocalWxyz = principalAxesLocalWxyz;
        }
      }
    }
...
export function parseXformOpFallbacksFromLayerText(
  layerText: string | null | undefined,
): Map<string, Map<string, XformOpValue>> {
  if (!layerText || typeof layerText !== "string" || !layerText.includes("xformOp:")) {
    return new Map();
  }

  const parsedByPrimPath = new Map<string, Map<string, XformOpValue>>();
  const contextStack: Array<{ name: string; path: string }> = [];
  let pendingContextName: string | null = null;
  ...
  const pushContext = (contextName: string) => {
  ...
  const parseXformOpValue = (opName: string, literal: string): XformOpValue | undefined => {
    if (!opName || !literal) return undefined;
    ...
    if (
      opName.startsWith("xformOp:rotateXYZ")
    ...
    if (
      opName.startsWith("xformOp:rotateX")
    ...
    if (opName.startsWith("xformOp:transform")) {
      if (numbers.length < 16) return undefined;
      return [
  ...
  const recordXformOpValue = (primPath: string, opName: string, literal: string) => {
    if (!primPath || !opName || !literal) return;
    const parsedValue = parseXformOpValue(opName, literal);
    if (parsedValue === undefined) return;

    let opMap = parsedByPrimPath.get(primPath);
    if (!(opMap instanceof Map)) {
      opMap = new Map<string, XformOpValue>();
      parsedByPrimPath.set(primPath, opMap);
    }
    opMap.set(opName, Array.isArray(parsedValue) ? ([...parsedValue] as XformOpValue) : parsedValue);
  };
  ...
  while ((lineMatch = lineRegex.exec(layerText))) {
    ...
    if (currentPath && trimmed.includes("xformOp:")) {
      ...
      if (xformMatch) {
        recordXformOpValue(
          currentPath,
          String(xformMatch[1] || "").trim(),
          String(xformMatch[2] || "").trim(),
        );
      }
    }
...
function toFiniteNumberLocal(value: unknown): number | undefined {
