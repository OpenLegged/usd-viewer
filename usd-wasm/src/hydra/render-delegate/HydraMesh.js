...
class HydraMesh {
    ...
    ensurePrimitiveFallbackGeometry() {
        ...
        switch (this._primitiveFallbackType) {
            ...
            case "sphere":
                ...
                break;
            case "cylinder":
                fallbackGeometry = new CylinderGeometry(0.5, 0.5, 1, PRIMITIVE_SEGMENTS.cylinderRadial, PRIMITIVE_SEGMENTS.cylinderHeight, false);
                break;
            case "collisionProxy":
    ...
    applyPrimitiveGeometryFromDescriptor(primType, descriptor) {
        ...
        if (normalizedType === 'cube') {
            ...
            generated = new SphereGeometry(Math.max(radius, 1e-6), PRIMITIVE_SEGMENTS.sphereWidth, PRIMITIVE_SEGMENTS.sphereHeight);
        }
            ...
            generated = new CylinderGeometry(Math.max(radius, 1e-6), Math.max(radius, 1e-6), Math.max(height, 1e-6), PRIMITIVE_SEGMENTS.cylinderRadial, PRIMITIVE_SEGMENTS.cylinderHeight, false);
            if (axis === 'X') {
            ...
            generated = new CapsuleGeometry(radius, capsuleBodyHeight, PRIMITIVE_SEGMENTS.capsuleCap, PRIMITIVE_SEGMENTS.capsuleRadial);
            if (axis === 'X') {
                generated.rotateZ(-Math.PI / 2);
            }
            else if (axis === 'Z') {
    ...
    applyResolvedPrimGeometryAndTransform(primPath) {
        ...
        if (!this.applyResolvedPrimGeometry(normalizedPrimPath))
            return false;
        const localXformOverride = this._interface.getCollisionLocalXformOverride(this._id);
        if (localXformOverride) {
            const proto = parseProtoMeshIdentifier(this._id);
    ...
    applyVisualColorOverride() {
        if (!this._id || this._id.includes('/collisions.'))
            return;
    ...
    _setMatrixFromRowMajorSource(matrixLike) {
        ...
        this._mesh.matrix.set(m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33);
    ...
    updateNormals(normals, profile = null) {
        ...
        if (this._geometry.hasAttribute('normal'))
            return;
