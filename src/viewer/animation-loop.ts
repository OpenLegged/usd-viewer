export interface AnimationFrameArgs {
  ...
  endTimeCode: number;
  shouldDraw?: () => boolean;
  drawBurstCount?: number;
  drawBurstBudgetMs?: number;
  frameDelayMs?: number;
  applyPostDrawTransforms?: (context: {
    didDraw: boolean;
    shouldRunDraw: boolean;
    drawFailed: boolean;
    controlsChanged: boolean;
  }) => boolean | void;
  applyMeshFilters: () => void;
  shouldApplyMeshFilters?: () => boolean;
  renderFrame: () => void;
}

export async function runAnimationFrame(args: AnimationFrameArgs): Promise<boolean> {
  const {
    driver,
    ready,
    timeout,
