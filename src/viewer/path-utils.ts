...
export function getSavedBooleanState(storageKey: string, fallback = false): boolean {
  try {
...
export function saveBooleanState(storageKey: string, enabled: boolean): void {
  try {
...
export function isSupportedUsdFileName(fileName = ""): boolean {
  return supportedUsdExtensions.has(getFileExtension(fileName));
}

export function normalizeUsdPath(inputPath: unknown, fallbackFileName = ""): string {
...
export function getDirectoryFromVirtualPath(filePath = "/"): string {
...
export function isTopLevelVirtualFilePath(filePath = ""): boolean {
