import {
  createDomElement,
  formatCollisionPrimitiveCounts,
  formatJointLimits,
  formatMass,
  formatVector3,
  getBasename,
  type RobotInspectorControllerOptions,
  type RobotJointMetadata,
  type RobotLinkMetadata,
  type RobotMetadataSnapshot,
} from "./robot-inspector/shared.js";

export {
  buildRobotMetadataSnapshot,
} from "./robot-inspector/shared.js";

export type {
  BuildRobotMetadataSnapshotArgs,
  RobotJointMetadata,
  RobotLinkMetadata,
  RobotMetadataTotals,
} from "./robot-inspector/shared.js";
...
export class RobotInspectorController {
  ...
  private readonly requestSnapshot: () => Promise<RobotMetadataSnapshot | null>;
  ...
  constructor(options: RobotInspectorControllerOptions) {
  ...
  async refresh(): Promise<void> {
    ...
    let snapshot: RobotMetadataSnapshot | null = null;
    try {
  ...
  private renderSnapshot(snapshot: RobotMetadataSnapshot): void {
    if (!this.list) return;
    this.list.innerHTML = "";
    ...
    summary.appendChild(createDomElement(
      "div",
      "robot-panel-metrics",
      `Links ${snapshot.totals.linkCount} · Joints ${snapshot.totals.jointCount} (ctrl ${snapshot.totals.controllableJointCount}) · Visual ${snapshot.totals.visualMeshCount} · Collision ${snapshot.totals.collisionMeshCount}`
    ));
    summary.appendChild(createDomElement(
      "div",
      "robot-panel-metrics",
      `Mass ${formatMass(snapshot.totals.totalMass)} · Mass links ${snapshot.totals.linksWithMass} · COM links ${snapshot.totals.linksWithCenterOfMass} · Inertia links ${snapshot.totals.linksWithInertia} · Materials ${snapshot.totals.materialCount}`
    ));
    summary.appendChild(createDomElement(
      "div",
      "robot-panel-metrics",
      `Collision primitives: ${formatCollisionPrimitiveCounts(snapshot.totals.collisionPrimitiveCounts)}`
    ));
  ...
  private createLinkTable(linkRecords: RobotLinkMetadata[]): HTMLElement {
    ...
    for (let index = 0; index < maxRows; index++) {
      ...
      const meshCountCell = createDomElement(
        "td",
        "",
        `${record.visualMeshCount}/${record.collisionMeshCount}${primitiveText === "-" ? "" : ` (${primitiveText})`}`
      );
  ...
  private createJointTable(jointRecords: RobotJointMetadata[]): HTMLElement {
    const table = document.createElement("table");
    table.className = "robot-panel-table";
    const header = document.createElement("thead");
    const headerRow = document.createElement("tr");
    ...
    for (let index = 0; index < maxRows; index++) {
      ...
      if (record.jointPath) jointCell.title = record.jointPath;
